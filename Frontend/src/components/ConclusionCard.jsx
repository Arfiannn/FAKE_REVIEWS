  ...
  if (isValid) {
    ...
    cardDescription = (
      <span>
        ...
        <span className={`font-extrabold px-1.5 py-0.5 rounded ${isPalsu ? 'text-rose-400 bg-rose-950/40 border border-rose-500/20' : 'text-emerald-400 bg-emerald-950/40 border border-emerald-500/20'}`}>
          {prediction_label || '-'}
        </span>{' '}
        dan hasil validasi AI Judge{' '}
        <span className="font-semibold text-slate-100">mendukung prediksi sistem</span> secara kuat.
      </span>
    );
  } else {
    // judge_verdict is NOT Valid ("Tidak Valid")
    cardTitle = 'Rekomendasi Peninjauan Ulang';
    themeClass = 'glow-amber border-amber-900/30 bg-amber-950/10';
    icon = <AlertTriangle className="w-10 h-10 text-amber-400 shrink-0" />;
    ...
    cardDescription = (
      <span>
        ...
        , namun hasil <span className="text-amber-400 font-bold underline decoration-amber-500/40 underline-offset-4">perlu ditinjau kembali</span> karena validasi AI Judge menunjukkan dukungan konteks masih{' '}
        <span className="font-semibold text-slate-100">lemah atau tidak selaras</span>.
      </span>
    );
  }
    ...
    <div className={`glass-panel rounded-2xl p-6 transition-all duration-300 ${themeClass} flex flex-col h-full justify-between`}>
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">KESIMPULAN AKHIR</span>
            <h3 className="text-lg font-bold text-slate-200">{cardTitle}</h3>
          </div>
        </div>

        {/* Dynamic User-Friendly Text */}
        <div className="bg-slate-950/60 rounded-xl p-4 md:p-5 border border-slate-900 text-slate-300 text-sm md:text-base leading-relaxed font-medium">
          {cardDescription}
        </div>
      </div>

      {/* Simplified User-Friendly Guide/Tip */}
      <div className="mt-6 pt-4 border-t border-slate-900/60 flex items-start gap-2.5 text-xs text-slate-400">
